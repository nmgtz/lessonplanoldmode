...
const studentCommentPatterns = {
    english: [
        ...
        (action, topic) => `Learners displayed clear understanding as they worked to ${action} ${topic}`,
...
// Patterns for introduction steps with verb conjugations
const introPatterns = {
    english: {
        askQuestion: [
        ...
        studentResponse: [
            ...
            (topic) => `Learners providing oral answers regarding ${topic}`,
...
// Patterns for teaching new knowledge with verb forms
const newKnowledgePatterns = {
    english: {
        teacher: [
            (action, topic) => `Teacher guiding students to ${action} ${topic}`,
            (action, topic) => `Instructor demonstrating how to ${action} ${topic}`,
            (action, topic) => `Teacher facilitating students' ability to ${action} ${topic}`,
            (action, topic) => `Educator leading the class in ${getVerbForm(action, 'gerund', 'english')} ${topic}`
        ],
        student: [
            (action, topic) => `Students working to ${action} ${topic}`,
            (action, topic) => `Learners practicing ${getVerbForm(action, 'gerund', 'english')} ${topic}`,
            (action, topic) => `Students actively ${getVerbForm(action, 'gerund', 'english')} ${topic}`,
        ...
        assessment: [
            ...
            (action, topic) => `Evaluating if students can successfully ${action} ${topic}`
        ]
    },
    swahili: {
        teacher: [
...
const reinforcementPatterns = {
    english: {
        teacher: [
            (action, topic) => `Teacher guiding students in groups to discuss how to ${action} ${topic}`,
            (action, topic) => `Instructor organizing group activities for ${getVerbForm(action, 'gerund', 'english')} ${topic}`,
            (action, topic) => `Teacher facilitating collaborative work to ${action} ${topic}`,
            (action, topic) => `Educator coordinating group discussions on ${getVerbForm(action, 'gerund', 'english')} ${topic}`
        ],
        student: [
            (action, topic) => `Students in groups discussing ways to ${action} ${topic}`,
            (action, topic) => `Learners collaborating in teams while ${getVerbForm(action, 'gerund', 'english')} ${topic}`,
...
const reflectionPatterns = {
    english: {
        teacher: [
            (action, topic) => `Teacher asking students about challenges faced when trying to ${action} ${topic}`,
            (action, topic) => `Instructor encouraging students to reflect on ${getVerbForm(action, 'gerund', 'english')} ${topic} in real-life contexts`,
            (action, topic) => `Teacher helping students connect ${getVerbForm(action, 'gerund', 'english')} ${topic} to practical situations`,
            (action, topic) => `Educator prompting students to relate their ${getVerbForm(action, 'noun', 'english')} of ${topic} to their experiences`
        ],
        student: [
            (action, topic) => `Students mentioning difficulties encountered while ${getVerbForm(action, 'gerund', 'english')} ${topic}`,
            (action, topic) => `Learners sharing how they can apply ${getVerbForm(action, 'gerund', 'english')} ${topic} in real situations`,
...
const consolidationPatterns = {
    english: {
        teacher: [
            (topic) => `Teacher asking a few questions and summarizing the lesson on ${topic}`,
            (topic) => `Instructor reviewing key points and concluding the lesson about ${topic}`,
            (topic) => `Teacher wrapping up with questions and summary of ${topic}`,
            (topic) => `Educator recapping the main concepts of ${topic} through questions`
        ],
        student: [
            (topic) => `Students answering questions and taking brief notes on ${topic}`,
            (topic) => `Learners responding to questions and writing key points about ${topic}`,
        ...
        assessment: [
            () => `Checking if students can answer questions correctly and take notes`,
            () => `Verifying students' ability to respond accurately and capture key points`,
            () => `Assessing whether learners can answer properly and document main ideas`,
...
if(wrdImp1 === "kuuliza maswali") {
    ...
    intro2.innerText = sentenceCase(getRandomElement(introPatterns.swahili.studentResponse)(specificLesson));
...
if(wrdImp1 === "ask question") {
    ...
    intro2.innerText = sentenceCase(getRandomElement(introPatterns.english.studentResponse)(specificLesson));
