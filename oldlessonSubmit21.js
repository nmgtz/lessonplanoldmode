...
class LessonPlanForm {
            ...
            <div class="success-page" style="text-align: center; padding: 40px 20px;">
                <div style="max-width: 600px; margin: 0 auto;">
                    <div style="width: 100px; height: 100px; background: #42b72a; display: flex; align-items: center; justify-content: center; margin: 0 auto 30px;">
                        <span style="color: white; font-size: 50px;"><i class="bi bi-check-circle-fill"></i></span>
                    </div>
                    <h2 style="color: #42b72a; margin-bottom: 20px;">Lesson Plan Created Successfully!</h2>
                    <p style="color: #65676b; font-size: 1.2rem; margin-bottom: 30px; text-transform: capitalize; font-weight: bold; font-family: verdana;">
                        Your lesson plan has been successfully created and saved!
                    </p>
                    
                    <div style="margin: 30px 0;">
                        <button id="createNewBtn" class="btn btn-primary" style="background: #1877f2; color: white; border: none; padding: 15px 30px; font-size: 1.1rem; cursor: pointer; margin-right: 15px;">
                            Create New Lesson Plan
                        </button>
                        <button id="viewFormBtn" class="btn btn-secondary" style="background: #e4e6eb; color: black; border: none; padding: 15px 30px; font-size: 1.1rem; cursor: pointer;">
                            View Form Data
                        </button>
                        <button id="clearDataBtn" class="btn btn-warning" style="background: #f7b928; color: black; border: none; padding: 15px 30px; font-size: 1.1rem; cursor: pointer; margin-left: 15px;">
                            Clear Saved Data
                        </button>
                    </div>
                    
                    <div style="margin-top: 30px; padding: 20px; background: #f7f8fa;">
                        <p style="color: #65676b; font-size: 0.9rem;">Thank you for using our lesson plan creator!</p>
                    </div>
                </div>
            </div>
        `;
    ...
    showFormData() {
        ...
        dataHTML += '<button onclick="history.back()" style="margin-top: 20px; padding: 10px 20px; background: #e4e6eb; color: black; border: none; cursor: pointer;">Back to Success Page</button>';
        dataHTML += '</div>';
...
// LESSON PLAN FOLDER SYSTEM - Remaining functions from second file...
// (I'll continue with the rest of the folder system functions)
...
function getCurrentFormData() {
    ...
    try {
        ...
        // **ADD THIS SECTION - Get edited reference value**
        const editableRefEl = document.getElementById("editableRef");
        const editableRefGroup = document.getElementById("editableRefGroup");
...
function loadDataIntoFormSilently(data) {
    try {
        ...
        // **ADD THIS SECTION - Restore edited reference**
        const editableRefGroup = document.getElementById('editableRefGroup');
        const editableRefEl = document.getElementById('editableRef');
        const refSelect = document.getElementById('refLesson');
        ...
        if (data.editableRef && data.editableRef.trim()) {
            ...
            // Wait for field to appear, then set the value
            setTimeout(() => {
                if (editableRefGroup) {
                    editableRefGroup.style.display = 'block';
                }
...
function addLessonViewModalStyles() {
    // Already included in main styles above
    return;
}
