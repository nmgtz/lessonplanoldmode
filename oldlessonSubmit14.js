...
if (dateOption) {
    dateOption.addEventListener('change', function() {
        ...
        this.form.addEventListener('input', (e) => {
            ...
            this.saveFormData(); // Save data on input
        });
        this.form.addEventListener('change', (e) => {
            this.clearError(e.target);
            this.saveFormData(); // Save data on change
        });
    ...
    // Additional validation for step 4 (students) - keep your existing validation
    if (step === 4) {
...
if (
    ...
    nextStep() {
        if (this.validateStep(this.currentStep)) {
            this.saveFormData(); // Save before moving to next step
    ...
    prevStep() {
        if (this.currentStep > 1) {
            this.saveFormData(); // Save before moving to previous step
            this.currentStep--;
            this.updateStep();
            this.saveCurrentStep();
        }
            ...
            <div class="success-page" style="text-align: center; padding: 40px 20px;">
                <div style="max-width: 600px; margin: 0 auto;">
                    ...
                    <div style="margin: 30px 0;">
                        ...
                        <button id="clearDataBtn" class="btn btn-warning" style="background: #ffc107; color: black; border: none; padding: 15px 30px; font-size: 1.1rem; border-radius: 8px; cursor: pointer; margin-left: 15px;">
                            Clear Saved Data
                        </button>
                    </div>
...
saveFormData() {
    ...
    // Method to manually save data (can be called from external functions)
    forceSave() {
        this.saveFormData();
    }

    // Method to get saved data status
    getSavedDataStatus() {
        try {
            const savedData = localStorage.getItem(this.storageKey);
            if (savedData) {
...
// Auto-focus functionality (unchanged)
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Add periodic save (every 30 seconds)
    setInterval(() => {
...
// Validate date only if user chose to set it
if (dateOption === 'set' && !date) {
    return false;
}
...
function sortLessonPlans(sortBy) {
    try {
        ...
        if (sortBy === 'date') {
            // **CHANGED: Sort newest first (descending)**
...
function displayFolderModal() {
    ...
    // **SORT NEWEST FIRST - Most recent timestamp at top**
    savedPlans.sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp));
...
function viewLessonPlanFromModal(lessonName) {
    ...
    // **INCREASED TIMEOUT: Wait longer for edited reference to be set**
    setTimeout(() => {
        ...
        // **ANOTHER DELAY: Wait for content to be fully generated**
        setTimeout(() => {
...
function getStreamValue() {
    ...
    if (!streamsSelect) return '';
    
    if (streamsSelect.value === 'custom' && customStreamInput) {
        return customStreamInput.value.trim();
    }
    
    return streamsSelect.value;
}
...
function getCurrentFormData() {
    ...
    try {
        ...
        // Add the actual stream value
        formData.actualStream = getStreamValue();
        
        // Add the actual period value
        formData.actualPeriod = getPeriodValue();
        
        // **FIXED: Get the edited reference value, not just the dropdown**
        const editableRefEl = document.getElementById("editableRef");
        const editableRefGroup = document.getElementById("editableRefGroup");
        
        if (editableRefGroup && editableRefGroup.style.display !== 'none' && editableRefEl && editableRefEl.value.trim()) {
            // Use the edited reference
            formData.refLesson = editableRefEl.value.trim();
            formData.editableRef = editableRefEl.value.trim();
        } else {
            // Use the dropdown value
            const refSelect = document.getElementById("refLesson");
            if (refSelect) {
                formData.refLesson = refSelect.value;
            }
        }
...
function generateLessonName(formData) {
    try {
        ...
        const dateOption = formData.dateOption;
        const date = formData.date;
        
       if (dateOption === 'set' && date) {
    return `${subject} - ${specificLesson} (${date})`;
} else {
    return `${subject} - ${specificLesson}`;
}
    } catch (error) {
...
function loadDataIntoForm(data) {
    try {
        ...
        // Restore custom stream if saved
        const streamsSelect = document.getElementById('streams');
        const customStreamInput = document.getElementById('customStream');
        const customStreamGroup = document.getElementById('customStreamGroup');
        
        if (data.streams === 'custom' && data.customStream) {
            customStreamGroup.style.display = 'block';
            customStreamInput.value = data.customStream;
            customStreamInput.required = true;
        }
        
        // Restore period if saved
        const periodTypeSelect = document.getElementById('periodType');
        const periodGroup = document.getElementById('periodGroup');
        const periodSelect = document.getElementById('period');
        
        if (data.periodType && data.period) {
            periodGroup.style.display = 'block';
            periodSelect.value = data.period;
            periodSelect.required = true;
        }
        
        // **FIXED: Restore edited reference if saved**
        const editableRefGroup = document.getElementById('editableRefGroup');
        const editableRefEl = document.getElementById('editableRef');
        const refSelect = document.getElementById('refLesson');
        
        if (data.editableRef && data.editableRef.trim()) {
            // First set the dropdown value
            if (refSelect && data.refLesson) {
                refSelect.value = data.refLesson;
                // Trigger change event to show the editable field
                refSelect.dispatchEvent(new Event('change', { bubbles: true }));
            }
            
            // Wait a moment for the field to appear, then set the value
            setTimeout(() => {
                if (editableRefGroup) {
                    editableRefGroup.style.display = 'block';
                }
                if (editableRefEl) {
                    editableRefEl.value = data.editableRef;
                }
            }, 100);
        }
...
function loadDataIntoFormSilently(data) {
    try {
        ...
        // First, load all regular form data
        Object.keys(data).forEach(key => {
            const element = document.getElementById(key) || form.querySelector(`[name="${key}"]`);
            if (element && data[key] !== undefined) {
                element.value = data[key];
        ...
        // **CRITICAL FIX: Handle edited reference BEFORE calling updateContent()**
        const editableRefGroup = document.getElementById('editableRefGroup');
        const editableRefEl = document.getElementById('editableRef');
        const refSelect = document.getElementById('refLesson');
        ...
        if (data.editableRef && data.editableRef.trim()) {
            ...
            // IMMEDIATELY show and populate the editable field (no setTimeout!)
            if (editableRefGroup) {
                editableRefGroup.style.display = 'block';
            }
            if (editableRefEl) {
            ...
            // Trigger change event AFTER setting everything
            if (refSelect) {
                refSelect.dispatchEvent(new Event('change', { bubbles: true }));
            }
        }
        ...
        if (data.streams === 'custom' && data.customStream) {
            if (customStreamGroup) {
                customStreamGroup.style.display = 'block';
            }
            if (customStreamInput) {
                customStreamInput.value = data.customStream;
            }
        }
...
function deleteAutoSavedVersions(lessonName) {
    try {
        const savedPlans = getSavedLessonPlans();
        
        // Filter out all auto-saved versions of this lesson
        const filteredPlans = savedPlans.filter(plan => {
            // Keep if: different name OR (same name but NOT auto-saved)
            return plan.name !== lessonName || !plan.autoSaved;
        });
        
        const deletedCount = savedPlans.length - filteredPlans.length;
        
        if (deletedCount > 0) {
            saveLessonPlansToStorage(filteredPlans);
            console.log(`🧹 Deleted ${deletedCount} auto-saved version(s) of "${lessonName}"`);
        }
        
        return deletedCount;
    } catch (error) {
        console.error('Error deleting auto-saved versions:', error);
        return 0;
    }
}
...
function manualSaveLessonPlan() {
    ...
    if (existingPlan) {
        showConfirmationModal({
            ...
            onConfirm: () => {
                // **NEW CODE: Delete all auto-saved versions before replacing**
                deleteAutoSavedVersions(lessonName);
                
                // Now update with the manual save
